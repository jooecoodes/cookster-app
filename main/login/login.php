            ...
            if ($response->success) {
                ...
                if (mysqli_num_rows($result) > 0) {
                    while ($rows = mysqli_fetch_assoc($result)) {
                        ...
                        $sqlUpdateQIDate = "UPDATE user SET quiz_indicator = 1 WHERE expected_update_qi <= ? AND id = ?";
                        $stmtUpdateQIDate = $conn->prepare($sqlUpdateQIDate);
                        $stmtUpdateQIDate->bind_param("si", $currentDateString, $_SESSION['userId']);
                        $stmtUpdateQIDate->execute();

                        if ($stmtUpdateQIDate->affected_rows > 0) {
                            echo "successfully edited qi";
                        } else {
                            echo "failed to edit qi because of wrong schedule";
                        }
